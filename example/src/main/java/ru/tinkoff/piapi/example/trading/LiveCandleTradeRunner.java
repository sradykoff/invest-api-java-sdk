...
public class LiveCandleTradeRunner {
    ...
    public void onCandle(Candle candle) {
        ...
        if (nextSignal.isBuy()) {
            ...
            var buyStatus = orderManager.buyLimit(nextSignal.getInstrumentId(), Quantity.ONE, nextSignal.closePrice);
            if (buyStatus.isSuccess()) {
                liveBot.onEnter(nextSignal, nextSignal.closePrice, Quantity.ONE);
            }
        } else if (nextSignal.isSell()) {
            log.debug("signal sell: {}", nextSignal);
            var sellStatus = orderManager.sellLimit(nextSignal.getInstrumentId(), Quantity.ONE, nextSignal.closePrice);
            if (sellStatus.isSuccess()) {
                liveBot.onExit(nextSignal, nextSignal.closePrice, Quantity.ONE);
            }
        } else {
